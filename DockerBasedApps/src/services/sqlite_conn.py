    ...
    def save_logs(self, logs: list[LogEntry] | LogEntry) -> None:
        ...
        with connect(self.__db_path) as conn:
            try:
                ...
                conn.executemany(
                    f"INSERT INTO {self.__logs_table} (timestamp, tag, message) VALUES (?, ?, ?)",  # noqa
                ...
                conn.rollback()
                raise ConnectionError(f"Error saving logs to database: {e}") from e  # noqa

    @lru_cache(maxsize=50)  # noqa # agregamos cache a las consultas a la base de datos.
    def get_logs(self, start_time: datetime, end_time: datetime) -> list[LogEntry]:
        ...
        with connect(self.__db_path) as conn:
            try:
                ...
                raise ConnectionError(f"Error retrieving logs from database: {e}") from e  # noqa
